...
func TestProductionAuthDeploymentContract(t *testing.T) {
	...
	require.Contains(t, stack.Services["nginx"].Volumes, "${FURANO_REPOSITORY_ROOT}/deploy/swarm/configs/nginx.conf:/etc/nginx/conf.d/default.conf:ro")
	...
	require.NotContains(t, stackYAML, "- ./", "production bind paths must not depend on Docker's relative-path resolution")
	require.NotContains(t, stackYAML, "- ../", "production bind paths must not depend on Docker's relative-path resolution")
