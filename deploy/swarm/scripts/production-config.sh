...
load_production_config() {
  ...
  unset LEGACY_CASSANDRA_VOLUME SWARM_CASSANDRA_VOLUME
  ...
  while IFS= read -r line || [[ -n "${line}" ]]; do
    ...
    value="${line#*=}"
    case "${key}" in
      STACK_NAME|PUBLIC_APP_ORIGIN|AUTH_MASTER_IMAGE|FURANO_IMPORT_IMAGE|FURANO_BACKEND_IMAGE|AUTH_POSTGRES_IMAGE|AUTH_SMTP_HOST|AUTH_SMTP_PORT|AUTH_MAIL_FROM|AUTH_SMTP_USER|FURANO_SUPERUSER|LEGACY_CASSANDRA_VOLUME|SWARM_CASSANDRA_VOLUME) ;;
