...
run_case() {
  ...
  PATH="${FAKE_BIN}:${PATH}" \
    ...
    AUTH_MASTER_IMAGE="${AUTH_IMAGE}" AUTH_POSTGRES_IMAGE="${POSTGRES_IMAGE}" \
    FURANO_BACKEND_IMAGE="${configured_backend}" \
    AUTH_SMTP_HOST="smtp.example.test" AUTH_MAIL_FROM="auth@example.test" \
...
assert_backend_preflight_failure() {
  ...
  grep -Fq "FURANO_BACKEND_IMAGE must be set to an immutable image digest" "${CASE_DIR}/output" ||
