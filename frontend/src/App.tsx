    ...
    return <div key={meta[meta_ind] + "_" + this.clade_name}
      ...
      <div style={{
        ...
        && <div style={{
          ...
          <CountButton {...{color: 'orange', number: this.childs_num}} />
        </div>}
        <TreeCladesAdapter {...{
...
class DataMeta {
  ...
  render_link(value: string) {
    return <>
      <a style={{fontSize: FONT_SIZE,}} href={this.additional_data + "/" + value}>{value}</a>
    </> // TO DO
  }

  render_cls(value: string) {
    return <>
      <p style={{fontSize: FONT_SIZE,}}>{value}</p>
    </> // TO DO
  }
    ...
    <tr key={"meta"}>
      { meta.map((curr_meta, ind) => (
        <th scope='col' key={"meta" + ind}>
          <p style={{fontSize: FONT_SIZE,}}>{curr_meta.name}</p>
        </th>
      ))}
    </tr>
  </thead>
    ...
    <>
      ...
      <div className="card">
        <form className="main_form" onSubmit={() => alert("your request is: " + request)}>
          <input type="text" className="search-teaxtarea" onChange={(text) => setRequest(text.target.value)} style={{fontSize: FONT_SIZE}}></input>
